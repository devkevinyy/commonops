...
func UpdateNodeMetrics(clusterId string, nodeMetrics *v1beta1.NodeMetricsList) (err error) {
	for _, v := range nodeMetrics.Items {
		...
		err = MysqlClient.Exec(insertNodeMetricsSql, v.UID, clusterId, v.Name, v.Usage.Cpu().MilliValue(), v.Usage.Memory().Value()/1024.0/1024.0, v.Usage.StorageEphemeral().Value()).Error
		if err != nil {
			return
		}
	}
...
func UpdatePodMetrics(clusterId string, podMetrics *v1beta1.PodMetricsList) (err error) {
	for _, v := range podMetrics.Items {
		for _, u := range v.Containers {
			...
			err = MysqlClient.Exec(insertPodMetricsSql, v.UID, clusterId, v.Name, v.Namespace, u.Name, u.Usage.Cpu().MilliValue(), u.Usage.Memory().Value()/1024.0/1024.0, u.Usage.StorageEphemeral().Value()).Error
			if err != nil {
				return
			}
		}
...
func CleanDatabase(window *time.Duration) (err error) {
	...
	err = MysqlClient.Exec(cleanNodeMetricsSql).Error
	if err != nil {
		return
	}

	cleanPodMetricsSql := "delete from pods where time <= date_sub(now(), interval 300 second);"
	err = MysqlClient.Exec(cleanPodMetricsSql).Error
	if err != nil {
		return
	}
	return
...
func getRows(clusterId string, table string, metricName string, selector ResourceSelector) (rows []MetricItem, err error) {
	...
	err = MysqlClient.Raw(query, values...).Scan(&rows).Error
	return
}
