...
func SaveOrUpdateEcs(instanceId string, cloudAccountId int, ecs Ecs) bool {
	...
	if count > 0 {
		result = database.MysqlClient.Model(&ecs).Where("instance_id=?", instanceId).Updates(&ecs)
	} else {
...
func AddCloudServer(data forms.ServerInfoForm) (err error) {
	ecs := Ecs{
		...
		SshPwd:               untils.DesEncode(data.SshPwd),
	}
...
func UpdateCloudServer(data forms.ExtraInfoForm) (err error) {
	...
	if data.EcsBaseInfoForm.SshPwd != "" {
		updateSql = fmt.Sprintf("%s, ssh_pwd='%s'", updateSql, untils.DesEncode(data.EcsBaseInfoForm.SshPwd))
	}
...
func GetEcsSshInfo(id string) (ecs Ecs, err error) {
	err = database.MysqlClient.Raw("select public_ip_address, ssh_port, ssh_user, ssh_pwd from ecs where data_status > 0 and id = ?", id).Scan(&ecs).Error
	return
}
