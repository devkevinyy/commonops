...
func SaveDmsDatabase(instanceId string, databaseId string, schemaName string, state string, dbType string, host string,
	...
	saveSql := fmt.Sprintf("INSERT INTO dms_database(instance_id, database_Id, schema_name, state, db_type, host, port) " +
		...
		"port = ?")
	err = database.MysqlClient.Exec(saveSql, instanceId, databaseId, schemaName, state, dbType, host, port, instanceId,
		schemaName, state, dbType, host, port).Error
	return
}

func GetDmsAllDatabaseData(instanceId string) (data []DmsDatabase, err error) {
	err = database.MysqlClient.Raw("select id, instance_id, database_id, schema_name, state, db_type, host, port "+
...
func GetDmsDatabaseInfoByDatabaseId(databaseId string) (databaseI DmsDatabase, err error) {
	err = database.MysqlClient.Raw("select instance_id, database_id, schema_name, db_type, host, port from dms_database where database_id = ?", databaseId).Scan(&databaseI).Error
	return
}

func DeleteDmsDatabase(instanceId string, databaseId string) (err error) {
	err = database.MysqlClient.Exec("update dms_database set data_status = 0 where instance_id = ? and database_id = ?", instanceId, databaseId).Error
	return
}