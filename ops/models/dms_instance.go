...
func SaveDmsInstance(port int, instanceType string, host string, state string, instanceId string,
	...
	err = database.MysqlClient.Exec(saveSql, port, instanceType, host, state, instanceId, instanceAlias, operUser, operPwd, port,
		instanceType, host, state, instanceAlias, operUser, operPwd).Error
	return
}

func DeleteDmsInstance(instanceId string) (err error) {
	err = database.MysqlClient.Exec("update dms_instance set data_status = 0 where instance_id = ?", instanceId).Error
	return
}

func GetDmsInstanceCount(instanceName string) (total int) {
	total = 0
...
func GetDmsInstanceList(instanceName string, offset int, limit int) (instanceList []DmsInstance) {
	...
	database.MysqlClient.Raw(querySql, args...).Scan(&instanceList)
	return
}

func GetDmsAllInstanceData() (instances []DmsInstance, err error) {
	err = database.MysqlClient.Raw("select instance_id, instance_type, instance_alias, host, port from dms_instance").Scan(&instances).Error
	return
}

func GetDmsInstanceByInstanceId(instanceId string) (instance DmsInstance, err error) {
	err = database.MysqlClient.Raw("select * from dms_instance where instance_id = ?", instanceId).Scan(&instance).Error
	return
}

func GetDmsInstanceOperUserInfoByInstanceId(instanceId string) (instance DmsInstance, err error) {
	err = database.MysqlClient.Raw("select oper_user, oper_pwd from dms_instance where instance_id = ?", instanceId).Scan(&instance).Error
	return
}
