...
func SaveUserFeedback(userId uint, username string, content string, score int) (err error) {
	...
	err = database.MysqlClient.Exec("insert into user_feedback(user_id, username, content, score, create_time) " +
...
func GetUserFeedbackCount() (total int) {
	total = 0
	database.MysqlClient.Model(&UserFeedback{}).Where("data_status > 0").Count(&total)
	return
}

func GetUserFeedbackList(offset int, pageSize int) (userFeedback []UserFeedback) {
	database.MysqlClient.Where("data_status > 0").Order("id desc").Offset(offset).Limit(pageSize).Find(&userFeedback)
	return
}