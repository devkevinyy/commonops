...
func AddUserDmsAuth(empId string, username string, authType int, instanceType string, instanceId string,
	...
	err = database.MysqlClient.Raw(querySql, empId, authType, instanceId, databaseId, operType).Count(&count).Error
	if err != nil {
		return
	}
	if count > 0 {
	...
	err = database.MysqlClient.Exec("insert into dms_auth(emp_id, username, auth_type, instance_type, "+
...
func GetUserAuthCount(empId string, instanceId string, operType string) (total int) {
	total = 0
...
func GetUserAuthList(empId string, instanceId string, operType string, offset int, limit int) (authList []DmsAuth) {
	...
	database.MysqlClient.Raw(querySql, args...).Scan(&authList)
	return
}
...
func DeleteUserDmsAuth(authId int) (err error) {
	err = database.MysqlClient.Exec("update dms_auth set data_status = 0 where id = ?", authId).Error
	return
}
...
func CheckUserDMSAuthDatabaseQuery(empId string, databaseId string, tableNames string) (hasAuth bool, err error) {
	var auth DmsAuth
	err = database.MysqlClient.Raw("select * from dms_auth where data_status = 1 and auth_type = 2 "+
...
func CheckUserDMSAuthDatabaseModify(empId string, databaseId string, tableNames string) (hasAuth bool, err error) {
	var auth DmsAuth
	var count int
	err = database.MysqlClient.Raw("select count(*) from dms_auth where data_status = 1 and auth_type = 2 "+
...
func GetUserInstanceData(empId string) (data []DmsAuth, err error) {
	err = database.MysqlClient.Raw("select emp_id, instance_type, instance_id, instance_name "+
...
func GetUserDatabaseData(empId string, instanceId string) (data []DmsAuth, err error) {
	err = database.MysqlClient.Raw("select database_id, database_name from dms_auth "+
...
func DecreaseUserOperatorCount(empId string, operType int, databaseId string) (err error) {
	err = database.MysqlClient.Exec("update dms_auth set oper_count = oper_count-1 where data_status = 1 and "+
...
func DmsBigTableProcedureCheck(host string, databaseName string, sqlContent string) (result ProcesureResult, err error) {
	err = database.MysqlClient.Raw("call p_dms_bigtable(?, ?, ?)", host, databaseName, sqlContent).Scan(&result).Error
	return
}

func DmsDeleteUserAuth(id string) (err error) {
	err = database.MysqlClient.Exec("update dms_auth set data_status = 0 where id = ?", id).Error
	return
}
