...
func GetUsersCount() (total int) {
	total = 0
	database.MysqlClient.Model(&User{}).Where("active=1").Count(&total)
	return
}

func GetUserInfo(email string) (user User, err error) {
	err = database.MysqlClient.First(&user, " active = 1 and email=?", email).Error
	return
}

func GetUserInfoByEmpId(empId string) (user User, err error) {
	err = database.MysqlClient.First(&user, " active = 1 and emp_id=?", empId).Error
	return
}

func UpdateUserFirstInStatus(id uint) {
	database.MysqlClient.Exec("update users set first_in=0 where id=?", id)
	return
}

func UpdateUserPassword(empId string, password string) (user User, err error) {
	err = database.MysqlClient.First(&user, "emp_id=?", empId).Error
	if err != nil {
...
func GetUserList(offset int, pageSize int) (users []User) {
	users = []User{}
	database.MysqlClient.Offset(offset).Limit(pageSize).Find(&users).Where("active=1")
	return
}

func GetRolesCount() (total int) {
	total = 0
	database.MysqlClient.Model(&Role{}).Count(&total)
	return
}

func GetRolesList(offset int, pageSize int) (roles []Role) {
	roles = []Role{}
	database.MysqlClient.Offset(offset).Limit(pageSize).Find(&roles)
	return
}

func GetUsersByRoleId(roleId int) (users []User, err error) {
	err = database.MysqlClient.Raw("SELECT ur.user_id as id FROM user_roles as ur "+
		"WHERE ur.role_id = ?", roleId).Scan(&users).Error
	return
}

func GetAllUsers() (users []User, err error) {
	err = database.MysqlClient.Raw("SELECT id, user_name, position FROM users where active = 1").Scan(&users).Error
	return
}
...
func AddNewRole(roleName string, description string) (err error) {
	...
	count := 0
	database.MysqlClient.Where("name=?", roleName).Find(&role).Count(&count)
	if count == 0 {
		database.MysqlClient.Create(&role)
		return
	} else {
...
func UpdateRole(id int, roleName string, description string) (err error) {
	err = database.MysqlClient.Exec("update roles set name=?, description=? where id=?",
...
func AddRoleUsers(roleId int64, userIdList []string) (err error) {
	...
	err = database.MysqlClient.Exec("delete from user_roles where role_id = ? "+
		"and user_id not in (?)", roleId, users).Error
	for _, userId := range userIdList {
		exist := 0
		database.MysqlClient.Raw("select count(*) from user_roles "+
			"where role_id = ? and user_id = ?", roleId, userId).Count(&exist)
		if exist == 0 {
			err = database.MysqlClient.Exec("insert into user_roles(role_id, user_id) "+
...
func GetAllAuthLinks() (permissions []Permissions, err error) {
	err = database.MysqlClient.Raw("SELECT id, name, url_path FROM permissions").Scan(&permissions).Error
	return
}

func GetAuthLinksByRoleId(roleId int) (permissions []Permissions, err error) {
	err = database.MysqlClient.Raw("SELECT permission_id as id FROM role_permissions "+
...
func AddAuthLink(name string, description string, path string) (err error) {
	...
	count := 0
	database.MysqlClient.Where("name = ? or url_path = ?", name, path).Find(&permission).Count(&count)
	if count == 0 {
		database.MysqlClient.Create(&permission)
		return
	} else {
...
func DeleteAuthLink(id int) (err error) {
	err = database.MysqlClient.Delete(&Permissions{}, "id=?", id).Error
	return
}

func AddRoleAuthLinks(roleId int64, authLinkIdList []string) (err error) {
	links := strings.Join(authLinkIdList, ",")
	err = database.MysqlClient.Exec("delete from role_permissions where role_id = ? "+
		"and permission_id not in (?)", roleId, links).Error
	for _, linkId := range authLinkIdList {
		exist := 0
		database.MysqlClient.Raw("select count(*) from role_permissions "+
			"where role_id = ? and permission_id = ?", roleId, linkId).Count(&exist)
		if exist == 0 {
			err = database.MysqlClient.Exec("insert into role_permissions(role_id, permission_id) "+
...
func GetPermissionsCount() (total int) {
	total = 0
	database.MysqlClient.Model(&Permissions{}).Count(&total)
	return
}

func GetPermissionsList(offset int, pageSize int) (permissions []Permissions) {
	permissions = []Permissions{}
	database.MysqlClient.Offset(offset).Limit(pageSize).Find(&permissions)
	return
}

func AddUser(username string, password string, email string, position string) (id uint, err error) {
	count := 0
	database.MysqlClient.Raw(fmt.Sprintf("select count(*) from users where email = '%s' ", email)).Count(&count)
	if count > 0 {
	...
	user := User{
		UserName: username,
		Password: untils.GenUserPassword(password),
		Email:    email,
		Active:   true,
		Position: position,
		EmpId:    untils.GetUUID(),
	}
	result := database.MysqlClient.Create(&user)
...
func UpdateUserActiveStatus(email string, active bool) (err error) {
	...
	err = database.MysqlClient.Exec(fmt.Sprintf("update users set active = %d where email = '%s' ",
		activeStatus, email)).Error
	return
}

func GetUserPermissions(userId int) (permissionList []Permissions) {
	database.MysqlClient.Raw("select distinct(p.id) as id, p.name, p.url_path from "+
		...
		"where ur.user_id = ?", userId).Scan(&permissionList)
	return
}
...
func IsUserSuperAdmin(userId uint) (isSuperAdmin bool) {
	count := 0
	database.MysqlClient.Raw("select count(*) from "+
...
// 用户是否状态正常
func IsUserValid(userId uint) (isValid bool) {
	count := 0
	database.MysqlClient.Raw("select count(*) from users where active=1 and id = ?", userId).Count(&count)
	if count > 0 {
		isValid = true
	} else {
		isValid = false
...
func GetIsUserHasPermision(userId uint, authLink string) int {
	count := 0
	database.MysqlClient.Raw("select count(distinct ur.user_id) from "+
		"permissions as p inner join role_permissions as rp on p.id = rp.permission_id "+
		"inner join user_roles as ur on rp.role_id = ur.role_id "+
