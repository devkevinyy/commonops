...
func GetOtherResCount(userId uint, queryKeyword string, queryResType string,
	queryCloudAccount int, queryExpiredTime string, queryManageUser int) (total int) {
...
func GetOtherRes(userId uint, offset int, limit int, queryKeyword string, queryResType string,
	queryCloudAccount int, queryExpiredTime string, queryManageUser int) (otherRes []OtherDetail) {
	isSuperAdminOrOps := IsUserSuperAdminOrOps(userId)
	nowTime := time.Now().Format("2006-01-02 15:04:05")
	querySql := ""
	var args []interface{}
	...
	database.MysqlClient.Raw(querySql, args...).Scan(&otherRes)
	return
}

func AddCloudOtherRes(data forms.AddOtherResForm) (err error) {
	if data.Cost == "" {
		data.Cost = "0.00"
	}
	if data.Id != 0 { // update
...
func GetAllOtherResList() (otherRes []OtherRes, err error) {
	err = database.MysqlClient.Raw("SELECT id, res_type, instance_name " +
...
func DeleteCloudOtherRes(id int) (err error) {
	err = database.MysqlClient.Exec("update other_res set data_status = 0 where id = ?", id).Error
	return err
}
