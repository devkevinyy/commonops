...
func SaveDmsSqlApprove(emdId string, username string, instanceType string, instanceId string, instanceName string,
	...
	createTime := untils.GetCurrentTime()
	...
	err = database.MysqlClient.Exec(insertSql, emdId, username, instanceType, instanceId, instanceName, databaseId,
...
func GetDmsUserApproveByPage(empId string, offset int, limit int) (approves []DmsApprove, err error) {
	...
	err = database.MysqlClient.Raw(querySql, empId, offset, limit).Scan(&approves).Error
	return
}

func GetApproveInfoById(id string) (approve DmsApprove, err error) {
	querySql := "select * from dms_approve where id = ?"
	err = database.MysqlClient.Raw(querySql, id).Scan(&approve).Error
	return
}

func SetApproveHasExecuted(id string) (err error) {
	err = database.MysqlClient.Exec("update dms_approve set has_executed = 1 where id = ?", id).Error
	return
}

func IsApproveCanExecute(id string) (canExecute bool, err error) {
	count := 0
	err = database.MysqlClient.Raw("select count(*) from dms_approve where id = ? and data_status = 1 "+
