...
type FeedbackListResp struct {
	Total int 							`json:"total"`
	Page int 							`json:"page"`
	Feedbacks []models.UserFeedback	    `json:"feedbacks"`
}
...
func IPostUserCreate(c *gin.Context){
	var req forms.UserForm
	err := c.BindJSON(&req)
	if err != nil {
		untils.Log.Error("[IPostUserCreate]", zap.String("用户参数异常", err.Error()))
		c.JSON(http.StatusBadRequest, untils.RespData{Code:-1, Msg: err.Error()})
		return
	}
	id, err := models.AddUser(req.Username, req.Password, req.Email, req.Position)
	if err != nil {
		untils.Log.Error("[IPostUserCreate]", zap.String("新增用户异常", err.Error()))
...
func ILogin(c *gin.Context){
	...
	if err != nil {
		c.JSON(http.StatusOK, untils.RespData{Code:-1, Msg:"用户不存在或被禁用"})
		return
	}
	inputPwd := untils.GenUserPassword(req.Password)
	if inputPwd != userInfo.Password {
		c.JSON(http.StatusOK, untils.RespData{Code:1, Msg:"密码校验错误"})
		return
	}
	data := map[string]interface{}{
		"token": untils.GenJWT(map[string]interface{}{
			"userId": userInfo.ID,
...
func ITokenRefresh(c *gin.Context){
	...
	if err != nil {
		c.JSON(http.StatusOK, untils.RespData{Code:-1, Msg:"用户不存在"})
		return
	}
	data := map[string]interface{}{
		"token": untils.GenJWT(map[string]interface{}{
...
func IUpdatePassword(c *gin.Context){
	empId, _ := c.Get("empId")
	var req forms.UpdatePasswordForm
	...
	if req.Password != req.ConfirmPassword {
		c.JSON(http.StatusOK, untils.RespData{Code: -1, Msg: "二次确认密码不一致", Data: nil})
		return
	}
	userInfo, err := models.UpdateUserPassword(empId.(string), untils.GenUserPassword(req.Password))
	if err != nil {
		c.JSON(http.StatusOK, untils.RespData{Code:-1, Msg: err.Error()})
		return
	}
	c.JSON(http.StatusOK, untils.RespData{Code:0, Msg:"success", Data: userInfo})
}
...
func IGetUserPermissions(c *gin.Context) {
	...
	permissionList := models.GetUserPermissions(int(userId.(float64)))
	c.JSON(http.StatusOK, untils.RespData{Code:0, Msg:"success", Data: permissionList})
}
...
func IGetRoleResourceList(c *gin.Context){
	...
	case "ecs":
		...
		if err1 != nil {
			c.JSON(http.StatusOK, untils.RespData{Code: -1, Msg: err1.Error(), Data: nil})
			return
		}
		if err2 != nil {
			c.JSON(http.StatusOK, untils.RespData{Code: -1, Msg: err2.Error(), Data: nil})
			return
		}
		data["in"] = inData
		data["all"] = allData
...
func IPostRoleResourcesList(c *gin.Context) {
	var req forms.AddRoleResources
	err := c.BindJSON(&req)
	if err != nil {
		c.JSON(http.StatusOK, untils.RespData{Code: -1, Msg: err.Error(), Data: nil})
		return
...
func IGetRoleAuthLinkList(c *gin.Context){
	...
	if !exist {
		untils.Log.Warn("userInfo 不存在")
	}
	untils.Log.Info("[IGetRoleAuthLinkList]", zap.Any("msg", userInfo))
	roleId, err := strconv.Atoi(c.Query("roleId"))
...
func ICreateRoleAuthLink(c *gin.Context) {
	var req forms.AddRoleAuthLinks
	err := c.BindJSON(&req)
	if err != nil {
		c.JSON(http.StatusOK, untils.RespData{Code: -1, Msg: err.Error(), Data: nil})
		return
...
func IPostUserFeedback(c *gin.Context) {
	var req forms.FeedbackForm
	err := c.Bind(&req)
	if err != nil {
		c.JSON(http.StatusOK, untils.RespData{Code: -1, Msg: err.Error(), Data: nil})
		return
	}
	userId := untils.GetCurrentUserId(c)
	...
	c.JSON(http.StatusOK, untils.RespData{Code: 0, Msg: "提交成功", Data: nil})
}
